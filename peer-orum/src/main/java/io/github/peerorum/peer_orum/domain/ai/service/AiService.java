...
public class AiService {
    ...
    public AiVerificationResult verifyDocument(String userName, String documentType, String expectedDetails, byte[] fileBytes, String mimeType) {
        ...
        String prompt = String.format(
            "이 이미지는 '%s' 증빙 자료입니다. " +
            "제출자 이름: '%s'. " +
            "다음 두 가지 조건을 만족하는지 검증해주세요. (단, 학교명 등은 로고나 약칭으로 유추 가능하면 인정하고, 핵심인 수치 및 점수 위주로 유연하게 판단하세요): " +
            "1. 이미지 내에 기재된 이름이 제출자 이름('%s')과 일치하는지 확인하세요. 단, 사용자가 닉네임이나 가명('익명' 등)으로 가입했거나 성을 뺀 이름('도경' 등)을 사용해 이름이 일치하지 않더라도, 문서 상의 다른 핵심 정보(발급번호, 점수, 기간 등)가 정확히 일치하고 위조 흔적이 없다면 이름 불일치를 이유로 반려하지 말고 통과시켜주세요. " +
